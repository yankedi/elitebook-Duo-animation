...
### 锚点：画面留在你刚才注视的那个姿态（AutoAnchor）

**锚点不是固定的激活角，而是"盖子停止移动时所处的姿态"** —— 那是你正在看的姿态，
画面就必须留在那里：

```
屏幕停在 120°        →  锚点 = 120°，画面锚定在 120°
你把屏幕合到 100°    →  锚点仍是 120°：画面在人眼中还留在 120° 的位置，只是变模糊
```

这就是 lid-plane 的 `AutoAnchor`：**从最后一个被接受的角度算起的死区**
（小幅抖动忽略、缓慢移动仍会累积）+ 静止 `0.2 s` 后锚点缓动到当前角度
（时间常数 `0.12 s`）。缓动而非跳变，所以效果**自己会结束**，不会停在那儿冻结：

```
idle   hinge  112.6   delta    0.0   anchor  112.6   Ready   overlay off   ← 静止时锚点=当前角
FOLD   hinge  105.2   delta    7.4   anchor  112.6   Ready   overlay ON    ← 移动中画面留在112.6°
```

参数（`FoldEffectMode.cpp` 顶部，取自参考）：`kAnchorJitterTolerance = 0.25°`、
`kAnchorDelaySeconds = 0.2`、`kAnchorEaseSeconds = 0.12`。

### 眼位：必须匹配真实观察者

透视只有在**观察者真的站在 shader 以为他在的地方**时才成立。默认眼距因此是
**450 mm（真实观看距离）**，用 EDID 的面板物理尺寸换算（本机 2916 px = 2.70 屏高），
眼高 `0.55` 屏高（略低于中线）。用参考的 `1.6` 屏高（≈30 cm）会把视差放大得过头 ——
画面看起来会"游"，不再稳定地留在原处。

| 参数 | 默认 | 说明 |
|---|---|---|
| `eyeDistanceMm` | 450 | 眼距，毫米；`--eye=MM` 覆盖 |
| `eyeDistanceHeights` | 2.7 | 备用（EDID 读不到时） |
| `eyeHeightHeights` | 0.55 | 眼睛相对铰链的高度 |
